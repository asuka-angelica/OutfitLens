...
public partial class MainWindow : Window
{
    ...
    void ChooseBase_Click(object sender, RoutedEventArgs e)
    {
        ...
        basePath = d.FileName; EmptyPreview.Visibility = Visibility.Collapsed; Render(); UpdateWorkflowState();
    }
    ...
    static OpenFileDialog Picker(bool multi)
    {
        ...
        if(!multi)
        {
            string starAsia=Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.MyPictures),
                "StarASIA");
            if(Directory.Exists(starAsia)) picker.InitialDirectory=starAsia;
        }
        return picker;
    }
    ...
    async void CaptureWindow_Click(object sender,RoutedEventArgs e) => await CaptureWardrobe();
    void StartContinuousCapture_Click(object sender,RoutedEventArgs e)
    {
        if(TargetWindowBox.SelectedItem is not WindowTarget selected)
        {
            MessageBox.Show("撮影するゲーム画面を一覧から選択してください。",
                "連続撮影モード",MessageBoxButton.OK,MessageBoxImage.Information);
            return;
        }

        var countText=new System.Windows.Controls.TextBlock
        {
            Text=$"追加済み：{items.Count}件",
            Foreground=(System.Windows.Media.Brush)FindResource("Muted"),
            Margin=new Thickness(0,4,0,12)
        };
        var captureButton=new System.Windows.Controls.Button
        {
            Content="撮影して追加",
        ...
        var compact=new Window
        {
            Title="Outfit Lens - 連続撮影",
            Width=350,
        ...
        compact.Left=Math.Max(SystemParameters.WorkArea.Left,
            SystemParameters.WorkArea.Right-compact.Width-20);
        compact.Top=SystemParameters.WorkArea.Top+20;
        captureButton.Click+=async (_,_) =>
        {
            captureButton.IsEnabled=false;
            if(await CaptureWardrobe()) countText.Text=$"追加済み：{items.Count}件";
            captureButton.IsEnabled=true;
        };
        finishButton.Click+=(_,_)=>compact.Close();
        compact.Closed+=(_,_)=>{Show();Activate();};
        compact.Show();
        Hide();
    }
    void RefreshTargets_Click(object sender,RoutedEventArgs e) => RefreshTargets();

    void RefreshTargets()
    {
        IntPtr previous=(TargetWindowBox.SelectedItem as WindowTarget)?.Handle ?? IntPtr.Zero;
        ...
        EnumWindows((handle,_) =>
        {
            ...
            if(text.Length>0) targets.Add(new WindowTarget(handle,text));
            return true;
        },IntPtr.Zero);
        TargetWindowBox.ItemsSource=targets.OrderBy(x=>x.Title).ToList();
        TargetWindowBox.SelectedItem=targets.FirstOrDefault(x=>x.Handle==previous)
            ?? targets.FirstOrDefault(x=>x.Title.Contains("ブループロトコル",StringComparison.OrdinalIgnoreCase))
            ?? targets.FirstOrDefault(x=>x.Title.Contains("Star",StringComparison.OrdinalIgnoreCase));
    }

    async Task<bool> CaptureWardrobe()
    {
        if(TargetWindowBox.SelectedItem is not WindowTarget selected)
        {
            MessageBox.Show("撮影するゲーム画面を一覧から選択してください。",
                "衣装部屋の撮影",MessageBoxButton.OK,MessageBoxImage.Information);
            return false;
        }
        Status.Text="指定したゲーム画面を撮影しています";
        try
        {
            IntPtr target=selected.Handle;
            if(!IsWindow(target)) throw new InvalidOperationException("選択したゲーム画面が見つかりません。");
            if(!GetWindowRect(target,out RECT rect))
                throw new InvalidOperationException("ゲームウィンドウを取得できませんでした。");
            Rectangle bounds=ToRectangle(rect);

            if(bounds.Width<320 || bounds.Height<240)
                throw new InvalidOperationException("撮影範囲が小さすぎます。ゲーム画面を手前に表示してください。");

            using var captured=new Bitmap(bounds.Width,bounds.Height,PixelFormat.Format32bppArgb);
            using(var graphics=Graphics.FromImage(captured))
            {
                ...
                try
                {
                    if(!PrintWindow(target,dc,2))
                        throw new InvalidOperationException("指定したゲーム画面を撮影できませんでした。");
                }
                finally { graphics.ReleaseHdc(dc); }
            }
            await AddWardrobe(captured);
            Render();
            UpdateWorkflowState();
            return true;
        }
    ...
    void RemoveItem_Click(object sender,RoutedEventArgs e){ if(((FrameworkElement)sender).Tag is OutfitItem i){items.Remove(i);i.IconBitmap?.Dispose();Render();UpdateWorkflowState();} }

    void UpdateWorkflowState()
    {
        if (BaseStepStatus==null || WardrobeStepStatus==null || ExportButton==null) return;
        BaseStepStatus.Text=basePath==null ? "未選択" : "✓ 完了";
        WardrobeStepStatus.Text=$"{items.Count}件";
        ExportButton.IsEnabled=basePath!=null && items.Count>0;
        Status.Text=basePath==null
            ? "まずゲーム内スクリーンショットを選択してください"
            : items.Count==0
                ? "次に衣装部屋の画面を追加してください"
                : "準備完了。このまま保存できます";
    }
    ...
    void Render()
    {
        ...
        Preview.Source=ToSource(rendered); UpdateWorkflowState();
    }

    void Export_Click(object sender,RoutedEventArgs e)
    {
        Render(); if(rendered==null){MessageBox.Show("先にゲーム内スクリーンショットを選択してください。");return;}
...
public sealed record WindowTarget(IntPtr Handle,string Title);
...
public partial class MainWindow : Window
{
    ...
    async void CaptureWindow_Click(object sender,RoutedEventArgs e) => await CaptureWardrobe(false);
    async void CaptureMonitor_Click(object sender,RoutedEventArgs e) => await CaptureWardrobe(true);
    void RefreshTargets_Click(object sender,RoutedEventArgs e) => RefreshTargets();
    ...
    async Task CaptureWardrobe(bool wholeMonitor)
    {
        if(TargetWindowBox.SelectedItem is not WindowTarget selected)
        {
            MessageBox.Show("撮影するゲーム画面を一覧から選択してください。",
        ...
        Status.Text="ゲーム画面を自動で開いて撮影しています";
        Hide();
        try
        {
            IntPtr target=selected.Handle;
            ...
            if(wholeMonitor)
            {
                ...
                if(monitor==IntPtr.Zero || !GetMonitorInfo(monitor,ref info))
                    throw new InvalidOperationException("モニターを取得できませんでした。");
                bounds=ToRectangle(info.rcMonitor);
            }
            else
            {
                RECT rect;
                int result=DwmGetWindowAttribute(target,9,out rect,Marshal.SizeOf<RECT>());
                if(result!=0 && !GetWindowRect(target,out rect))
                    throw new InvalidOperationException("ゲームウィンドウを取得できませんでした。");
